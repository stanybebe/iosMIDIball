...
void ofApp::setup() {
    ...
    h = ofGetHeight()/2;
    sliderA.setup();
    bb.setup(2,w-50,650,w-200, 600, 400);
...
void ofApp::update() {
  ...
  //  if (padA.value == true){
       ofPoint wind(knobA.value,0);
     
        
   // }
    ofPoint grav(0,knobB.value);
    bb.applyForce(wind);
    bb.applyForce(grav);
    bb.update();
    bb.edgeCheck();
...
void ofApp::draw() {
    ...
    t = t+1;
    size=2;
    knobA.draw(w-300,1700,1);
    knobB.draw(w+300,1700,1);
    ...
    if( branch == 0){
           ...
           ofDrawTriangle(600, 600, 700, 600, 650, 500);
...
//    cout << branch <<  endl;
    ...
    if ( yep == true){
           ...
           //midiOut.sendNoteOn(2,array1[i],127);
               ...
               outputs[i]->sendNoteOn(2,note+sliderA.value);
               yep=false;
